...
class VerticalSpecimenPlace(private val elevator: Elevator, private val verticalArm: VerticalArm, private val verticalWrist: VerticalWrist) : SequentialCommandGroup() {
    init {
        addRequirements(elevator, verticalArm, verticalWrist)

        addCommands(
            ParallelCommandGroup(
                ElevatorPIDCommand(elevator, VerticalConstants.ElevatorPositions.BOTTOM + 12.0),
                VerticalArmCommand(verticalArm, VerticalConstants.VerticalArmPositions.SPECIMEN),
                VerticalWristCommand(verticalWrist, VerticalConstants.VerticalWristPositions.SPECIMEN_PLACE)
            )
        )
    }
}