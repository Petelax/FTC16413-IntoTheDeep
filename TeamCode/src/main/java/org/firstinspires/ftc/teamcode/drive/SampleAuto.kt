...
class SampleAuto : OpMode() {
        ...
        Parallel(
            Sequential(
                ...
                IfElse ( {HorizontalExtension.getPosition() < 1.0},
                    Parallel(HorizontalExtension.waitUntilSetPoint(HorizontalConstants.HorizontalExtensionPositions.CLEAR), HorizontalExtension.pid(HorizontalConstants.HorizontalExtensionPositions.CLEAR)),
                    Sequential()
                ),
            ),
            HorizontalArm.outHorizontalArm(),
            HorizontalWrist.outHorizontalWrist(),
            VerticalArm.intake(),
            VerticalWrist.intake(),
        )
    ...
    private val tenth = PurePursuitController.waypointsToPath(listOf(
        CurvePoint(Pose2d(20.0, 20.0, Rotation2d.fromDegrees(-135.0)), 1.0, 1.0, 6.0),
        CurvePoint(Pose2d(30.0, 62.0, Rotation2d.fromDegrees(0.0)), 1.0, 1.0, 6.0),
        CurvePoint(Pose2d(48.0, 62.0, Rotation2d.fromDegrees(0.0)), 1.0, 1.0, 6.0),
    ), kSmooth = 0.95)

    val auto = Sequential(
        Timeout(SwerveDrivetrain.alignModules(Pose2d(20.6, 20.6, Rotation2d.fromDegrees(-135.0))), 0.2),
        ...
        Parallel(
            ...
            SwerveDrivetrain.bp2p(Pose2d(20.6, 20.6, Rotation2d.fromDegrees(-135.0)), 2.0)
        ),
        ...
        verticalRetract,
        Parallel(
            Timeout(PurePursuitController.followPathCommand(eighth, constants = DrivebaseConstants.PurePursuitPIDCoefficients.copy(TranslationPositionTolerance = 1.0, RotationPositionTolerance = 0.06)), 2.0),
            Sequential(
                Wait(1.3),
                Timeout(intake, 1.0),
                Parallel(
                    HorizontalExtension.waitUntilSetPoint(HorizontalConstants.HorizontalExtensionPositions.SPRING_START),
                    HorizontalExtension.pid(HorizontalConstants.HorizontalExtensionPositions.SPRING_START),
                ),
                Wait(0.2),
                Race(
                    Timeout(Intake.runIntakeStopping().then(Intake.backDrive()), 2.0),
                    Sequential(
                        HorizontalExtension.spin(0.3),
                        Wait(3.0),
                        HorizontalExtension.spin(0.0),
        ...
        Parallel(
            Timeout(PurePursuitController.followPathCommand(ninth), 3.0),
            sample,
        ),
        Sequential(
            Deposit.open(),
    ...
    override fun init() {
        ...
        Intake.setPower(0.0)
        HorizontalWrist.setPosition(HorizontalWrist.cachedPosition)
        HorizontalArm.setPosition(HorizontalArm.getCachedPosition())
